...
import { getOficinas, getTramites } from '../services/api';
...
const OficinasAtencion = (props) => {
  ...
  const searchOficina = (oficinaToSearch) => {
    if (oficinas != null) {
      const filteredOficces = oficinas.filter((item) => item.descripcion.toLowerCase().includes(oficinaToSearch.toLowerCase()));
  ...
  const createOficinaMasUsada = () => {
    if (oficinas != null) {
      const offices = oficinas.slice(0, 3); // Corta la variable para mostrar solo 3 resultados.
              ...
              route != null ? (
                <MapboxGL.ShapeSource id="routeSource" shape={route.routes[0].geometry}>
                  <MapboxGL.LineLayer
                    id="routeFill"
                    style={{
                      lineColor: '#ff8109', lineWidth: 3.2, lineCap: MapboxGL.LineJoin.Round, lineOpacity: 1.84,
                    }}
                  />
